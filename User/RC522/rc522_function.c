...
uint8_t ReadRawRC(uint8_t ucAddress)
{
  ...
  ucAddr = ((ucAddress << 1) & 0x7E) | 0x80; // 手册中规定 bit 7读写 bit6-1为地址 bit0必须为0
  RC522_CS_Enable();
...
/**
 * @brief  通过RC522和ISO14443-A协议 卡通讯
 * @param  ucCommand，RC522命令字
 * @param  pInData，通过RC522发送到卡片的数据 14443
...
char PcdAnticoll(uint8_t *pSnr)
{
  ...
  if (cStatus == MI_OK) // 通信成功
  {
    ...
    if (ucSnr_check != ucComMF522Buf[uc]) // 最后一位为校验位
      cStatus = MI_ERR;
  }
                  ...
                  uint8_t *pSnr)
{
  ...
  ucComMF522Buf[0] = ucAuth_mode; // 认证命令代码
  ...
  cStatus = PcdComMF522(PCD_AUTHENT, // 认证和接收不相同
                        ucComMF522Buf,
                        12,
                        ucComMF522Buf,
                        &ulLen);
...
char PcdWrite(uint8_t ucAddr, uint8_t *pData)
{
  ...
  ucComMF522Buf[0] = PICC_WRITE; // 写操作
