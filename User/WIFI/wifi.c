...
#include "string.h"
#include "./WIFI/wifi.h"			 //包含需要的头文件
#include "./usart/bsp_debug_usart.h" //包含需要的头文件
...
void WiFi_ResetIO_Init(void)
{
	...
	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOG, ENABLE); // 使能PE端口时钟

	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2;		  // 准备设置PE2
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz; // 速率50Mhz
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT; // 推免输出方式
	GPIO_Init(GPIOE, &GPIO_InitStructure);		  // 设置PE2
	RESET_IO(1);								  // 复位IO拉高电平

	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_15; // 准备设置PG15
	GPIO_Init(GPIOG, &GPIO_InitStructure);	   // 设置PG15
}
/*-------------------------------------------------*/
/*函数名：WiFi发送设置指令                         */
/*参  数：cmd：指令                                */
...
char WiFi_Get_LinkSta(void)
{
	...
	else if (strstr(WiFi_RX_BUF, "CLOSED"))
	{ // 如果接受到CLOSED表示有链接断开
		sscanf(WiFi_RX_BUF, "%[^,],%[^,]", id_temp, sta_temp);
		printf("有客户端断开,ID=%s\r\n", id_temp); // 串口显示信息
		WiFi_RxCounter = 0;						   // WiFi接收数据量变量清零
...
char WiFi_SendData(char id, char *databuff, int data_len, int timeout)
{
	...
	{											  // 没超时，正确
		...
		{				  // 等待超时与否
			...
			if (strstr(WiFi_RX_BUF, "link is not valid"))
			{											  // 如果接受link is not valid，表示连接断开
				WiFi_RxCounter = 0;						  // WiFi接收数据量变量清零
				memset(WiFi_RX_BUF, 0, WiFi_RXBUFF_SIZE); // 清空WiFi接收缓冲区
				return 2;								  // 返回2
			}
...
char WiFi_Close(int timeout)
{
	...
	printf("\r\n"); // 串口输出信息
	if (timeout <= 0)
		return 1; // 如果timeout<=0，说明超时时间到了，返回1
	return 0;	  // 正确，返回0
}
/*-------------------------------------------------*/
/*函数名：初始化wifi模块                           */
