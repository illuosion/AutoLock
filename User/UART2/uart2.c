...
uint16_t Usart3_RxCounter; // Usart2 接收到的数据长度计数器
...
void uart3_init(void)
{
	...
	/*使能内部外设时钟，uart3的时钟只能分开写，否则初始化不正常*/
...
// 		; // 检查DMA发送通道内是否还有数据
// 	if (buffer)
// 		memcpy(Usart2_TxBuff, buffer, (size > USART2_TXBUFF_SIZE ? USART2_TXBUFF_SIZE : size)); // 判断发送长度是否大于DMA可传输长度
...
{
	USART_SendData(USART3, data); // 串口2发送数据
