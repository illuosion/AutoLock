...
// MCU对液晶屏的操作实际上就是吧显示数据写入到显存中，FSMC可以用于控制扩展的外部存储器
// 根据液晶扫描方向而变化的XY像素宽度
...
__inline void ILI9806G_Write_Data(uint16_t usData)
{
	*(__IO uint16_t *)(FSMC_Addr_ILI9806G_DATA) = usData;
	// uint16_t *p = FSMC_Addr_ILI9806G_DATA;  等价
	// *p = usData;
}
...
static void ILI9806G_FSMC_Config(void)
{
	...
	FSMC_NORSRAMTimingInitTypeDef readWriteTiming; // 读写寄存器
	...
	readWriteTiming.FSMC_AccessMode = FSMC_AccessMode_B; // A为SRAM

	/*以下配置与模式B无关*/
	// 地址保持时间（ADDHLD）模式A未用到
	readWriteTiming.FSMC_AddressHoldTime = 0x00; // 地址保持时间
	// 设置总线转换周期，仅用于复用模式的NOR操作
	readWriteTiming.FSMC_BusTurnAroundDuration = 0x00; // 地址线是否切换为数据线
	// 设置时钟分频，仅用于同步类型的存储器
	readWriteTiming.FSMC_CLKDivision = 0x00; // SRAM为异步默认不使用
	// 数据保持时间，仅用于同步型的NOR
	readWriteTiming.FSMC_DataLatency = 0x00; // SRAM为异步默认不使用

	FSMC_NORSRAMInitStructure.FSMC_Bank = FSMC_Bank1_NORSRAMx;					   // NORSRAM3
	FSMC_NORSRAMInitStructure.FSMC_DataAddressMux = FSMC_DataAddressMux_Disable;   // 不分时复用
	FSMC_NORSRAMInitStructure.FSMC_MemoryType = FSMC_MemoryType_NOR;			   // 存储器类型NOR_FLASH
	FSMC_NORSRAMInitStructure.FSMC_MemoryDataWidth = FSMC_MemoryDataWidth_16b;	   // IO线数据宽度
	FSMC_NORSRAMInitStructure.FSMC_BurstAccessMode = FSMC_BurstAccessMode_Disable; // 突发访问模式 --给定地址后数据可以连续访问  SRAM不支持  对液晶屏访问固定地址
	FSMC_NORSRAMInitStructure.FSMC_WaitSignalPolarity = FSMC_WaitSignalPolarity_Low;
	FSMC_NORSRAMInitStructure.FSMC_WrapMode = FSMC_WrapMode_Disable;
	FSMC_NORSRAMInitStructure.FSMC_WaitSignalActive = FSMC_WaitSignalActive_BeforeWaitState;
	FSMC_NORSRAMInitStructure.FSMC_WriteOperation = FSMC_WriteOperation_Enable; // 可读可写
	FSMC_NORSRAMInitStructure.FSMC_WaitSignal = FSMC_WaitSignal_Disable;
	FSMC_NORSRAMInitStructure.FSMC_ExtendedMode = FSMC_ExtendedMode_Disable; // 不使用则读写均为BRWT,使用则BRWT和BRT都适用
	FSMC_NORSRAMInitStructure.FSMC_WriteBurst = FSMC_WriteBurst_Disable;
	FSMC_NORSRAMInitStructure.FSMC_ReadWriteTimingStruct = &readWriteTiming; // 指向同一个变量
